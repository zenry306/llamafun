...
<?php
	...
	if(isset($_GET["report_member_id"]) && !empty($_GET["report_member_id"])){ //顯示member_id所有的惡意檢舉
		$report_member_id = $_GET["report_member_id"];
		$sql_report = mysqli_query($con, "SELECT * FROM report WHERE report_judge='guilty' AND report_member_id='$report_member_id' ORDER BY date DESC");
		echo "<div class='page_title'>".$report_member_id."</div>";
	}
	else{
		$sql_report = mysqli_query($con, "SELECT * FROM report WHERE report_judge='guilty' ORDER BY date DESC");
	}

?>
<div class="page_title">有罪</div>
