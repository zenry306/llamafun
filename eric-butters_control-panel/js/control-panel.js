$(function(){
	...
	$(document).on('click','.report-judge',function(){
		...
		*/
		...
		$.ajax({
			type:'POST',
			url:'/eric-butters_control-panel/report_judge-db.php',
	...
	$(document).on('click','.ban-submit',function(){
		...
		$.ajax({
			type:'POST',
			url:'/eric-butters_control-panel/ban-account.php',
	...
	$(document).on('click','.unban-submit',function(){
		...
		$.ajax({
			type:'POST',
			url:'/eric-butters_control-panel/unban-account.php',
	...
	$(document).on('keyup', '.thread_uploadimg_url', function() {
		...
		globalTimeout_thread = setTimeout(function() {
			...
			if(img_url.length>6){
				...
				$.ajax({
					type:'POST',
					url:'/eric-butters_control-panel/uploadimg-url-check.php',
	...
	$(document).on('click', '.newthread-send', function() {
		...
		if(SUBJECT.length < 1){
		 ...
		 if(format_correct==true){
			...
			$.ajax({
				type:'POST',
				url:'/eric-butters_control-panel/newthread-cp-db.php',
				data:formdata,
