...
<?php 
	...
	if(isset($_GET["post_member_id"]) && !empty($_GET["post_member_id"])){


		$post_member_id = $_GET["post_member_id"];

		$sql_checkmember_exist = mysqli_query($con, "SELECT  member_id  FROM members WHERE member_id='$post_member_id' ");
		$row_checkmember_exist = mysqli_num_rows($sql_checkmember_exist);
		if($row_checkmember_exist<1){
		...
		$sql_member_allpost = mysqli_query($con, "SELECT  *  FROM threads WHERE member_id='$post_member_id' LIMIT 11"); //LIMIT 設定為11比10多就表示有更多文章
		$rows_member_allpost = mysqli_num_rows($sql_member_allpost);
		
		$showLimit = 10;
		$sql_thread = "SELECT  *  FROM threads WHERE member_id='$post_member_id' ORDER BY date DESC LIMIT ".$showLimit;
		$result_thread = mysqli_query($con, $sql_thread);
		
		$sql_post_username = mysqli_query($con, "SELECT username,total_like FROM member_profile WHERE member_id = '$post_member_id' ");
		$row = mysqli_fetch_assoc($sql_post_username);
		$title_username = $row['username'];
		$title_like = $row['total_like'];
	...
	?>	
		<title>LlamaFUN - <?php echo $title_username;?> </title>
<div class="container">

	<?php include_once (dirname(__FILE__).'/rightsidebar.php');?>

<div class="move-div-table">
	<div class="member_postthreads_title"> <?php echo $title_username."的全部文章";?> </div>
	<div class="title_total_like"><?php echo "拉馬 - ".$title_like."隻";?></div>
	<div class="div-table">
	...
	?>
	  ...
	  <div class="div-thread">
		...
		<?php include (dirname(__FILE__).'/thread-features.php'); //載入按讚、分享、留言等功能?>
				...
				<!--<button type="button" id = "<?php //echo $thread_id; ?>" class="submit-comment" >發表</button>!-->
				<!--<input accept="image/gif, image/jpeg , image/png" type="file" id="comment_image_<?php //echo $thread_id; ?>" />!-->
						...
						if (preg_match_all($regex, $comments, $member_id_arr)) {
							...
							for($i=0 ; $i<count($member_id_arr[1]) ; $i++){
								...
								if($rows_member_id_exist>0){
									//echo $member_id;
									$sql_at_member_id = mysqli_query($con, "SELECT username FROM member_profile WHERE member_id='$comments_member_id'");
	...
	<?php } 
		if( $rows_member_allpost > $showLimit ){
	?>
