...
<div class="index-ads">
 ...
 <?php
	...
	if(isset($_POST["thread_type"]) && !empty($_POST["thread_type"])&& 
		...
		if($thread_type==="hot"){
			...
			//echo $hotness;
			$sql_thread = "SELECT  * , LOG10(thread_like + 1) * 287015 + UNIX_TIMESTAMP(date) AS Hotness  FROM threads WHERE LOG10(thread_like + 1) * 287015 + UNIX_TIMESTAMP(date) < '$hotness' ORDER BY Hotness  DESC LIMIT ".$showLimit;
		}
		else if($thread_type==="fresh"){
			$sql_thread = "SELECT  *  FROM threads WHERE date<'$date' ORDER BY date DESC LIMIT ".$showLimit;
			if(isset($_POST["post_member_id"]) && !empty($_POST["post_member_id"])){
				$post_member_id = $_POST["post_member_id"];
				$sql_thread = "SELECT  *  FROM threads WHERE date<'$date' AND member_id='$post_member_id' ORDER BY date DESC LIMIT ".$showLimit;
			}
		}
		...
		while($row = mysqli_fetch_array($result_thread)) {
			...
			$image_path = $image; //不是GIF的話
			$likecount = $row['thread_like'];
			$who_like = $row['who_like'];
			$thread_nft_onoff = $row['nft_onoff'];
			//$threadurl = "http://".$_SERVER['SERVER_NAME']."/llamafun/thread-show.php?thread_id=".$thread_id;
...
<?php
		...
		if($thread_type==="hot"){
			$hotness = $_POST["hotness"];
			$sql_thread = "SELECT  * , LOG10(thread_like + 1) * 287015 + UNIX_TIMESTAMP(date) AS Hotness  FROM threads WHERE LOG10(thread_like + 1) * 287015 + UNIX_TIMESTAMP(date) < '$hotness' ORDER BY Hotness  DESC LIMIT 1"; //搜尋HOT文章是否還有更舊的
		}
		else if($thread_type==="fresh"){
			$sql_thread = "SELECT  *  FROM threads WHERE date<'$date' ORDER BY date DESC LIMIT 1"; //搜尋FRESH文章是否還有更舊的
			if(isset($post_member_id) && !empty($post_member_id)){
				$sql_thread = "SELECT  *  FROM threads WHERE date<'$date' AND member_id='$post_member_id' ORDER BY date DESC LIMIT 1"; //搜尋FRESH文章是否還有更舊的
			}
		}
		$sql_thread_all = mysqli_query($con, $sql_thread);
		$all_Rows = mysqli_num_rows($sql_thread_all);
		if($all_Rows>0){
?>
