 <?php
	...
	if(isset($_SESSION['member_id']) && !empty($_SESSION['member_id']) && isset($_POST["nft_date"]) ){
...
<?php			
			...
			$sql_notification_check = mysqli_query($con, "SELECT * FROM notification WHERE rcv_member_id='$member_id' AND date<'$date' LIMIT 1" ); //搜尋是否還有更多舊通知，設定最多結果為1 節省資源，只有要一筆以上就表示還有
			$rows_notification_check = mysqli_num_rows($sql_notification_check);
			if($rows_notification_check > 0 ){
?>
