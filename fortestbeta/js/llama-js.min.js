  ...
  (function(d, s, id){
	...
	//機器人驗證-登入
	var reCAPTCHA_LOGIN_STATUS = false;
	var reCAPTCHA_LOGIN_CALLBACK = null;
	var verifyCallback_login = function(response) {
		reCAPTCHA_LOGIN_STATUS = true;
		reCAPTCHA_LOGIN_CALLBACK = response;
		/*
		$.ajax({
			type:'POST',
			url:'/fortestbeta/recaptcha.php',
			data:{secret :"6LekoDEUAAAAAJCS4Mgn1dhBQxozE_C9Y-h8Bt94" , response:response  },
	...
	var expiredCallback_login = function() {
		reCAPTCHA_LOGIN_STATUS=false;
	};
	
	//機器人驗證-註冊
	var reCAPTCHA_REG_STATUS = false;
	var reCAPTCHA_REG_CALLBACK = null;
	var verifyCallback_reg = function(response) {
		reCAPTCHA_REG_STATUS = true;
		reCAPTCHA_REG_CALLBACK = response;
	/*
		$.ajax({
			type:'POST',
			url:'/fortestbeta/recaptcha.php',
			...
			success:function(data){
				...
				if(data==="success"){
					...
					reCAPTCHA_REG_STATUS = false;
				}
				
			}
		});*/
	};
	var expiredCallback_reg = function() {
		reCAPTCHA_REG_STATUS=false;
	};
	...
	var onloadCallback = function() {
		...
		reCAPTCHA_LOGIN = grecaptcha.render(document.getElementById('reCAPTCHA_LOGIN'), {
			'sitekey' : '6LekoDEUAAAAAGu6IU0YaFRPSlNkuC2JtkDZrXeJ',
			'callback' : verifyCallback_login,
			'expired-callback' : expiredCallback_login,
			'theme' : 'light'
		});
		reCAPTCHA_REG = grecaptcha.render(document.getElementById('reCAPTCHA_REG'), {
			'sitekey' : '6LekoDEUAAAAAGu6IU0YaFRPSlNkuC2JtkDZrXeJ',
			'callback' : verifyCallback_reg,
			'expired-callback' : expiredCallback_reg,
			'theme' : 'light'
		});
		/*
...
$(function(){
	...
	$(window).on('click',function(event) {
		...
		else if (event.target == forgotpw_popup) { //點擊forgotpw_popup的任何地方都可以關閉登入視窗
			$('body').css('overflow','auto');
			$('.forgotpw_popup').css('display','none');
		}
		...
		else if (event.target == side_nav_popup) { //點擊leaderboard_popup的任何地方都可以關閉排行榜
			$('body').css('overflow','auto');
			$('#side-nav').width('0');
			$('.side_nav_popup').css('display','none');
		}
	});
	...
	//});
	//alert('~');
	$(window).resize(function() { //拉動視窗後關閉popup排行榜
		//$('#side-nav').hide();
		//$('#side-nav').width('0');
		if( w != $( window ).width() ){
	...
	$(document).on('click','.toggle-nav',function() {
		...
		$('.side_nav_popup').css('display','block');
		$('#side-nav').width('250');
			...
			if($('.thread-loding').length){
				...
				*/
				...
				if(THREADLOAD_FINISH===true){
					...
					$.ajax({
						type:'POST',
						url:'/fortestbeta/thread-show-more.php',
	...
	$(document).on('click','.contact-us',function(){
		$('.contact_finished').css('display','none');//每次開啟先關閉contact_finished
	...
	$(document).on('click', '.contact-submit', function() {
		var sub_con_format = /(?!^\s+$)^(.|\n)+$/;//不允許全部空白無任何文字或符號，但文字之間可以有空白
		var username_format = /^[\u4e00-\u9fa5\w-]+$/; //不能使用注音　符號只能使用-或_
		var mail_format = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
		var USERNAME = document.getElementById("contact_username").value;
		var EMAIL = document.getElementById("contact_email").value;
		var CATEGORY = $('.contact_category').find(":selected").text();
		var SUBJECT = document.getElementById("contact_subject").value;
		var CONTENT = document.getElementById("contact_content").value;
		var check = true;

		$('.contact_errormsg').css('visibility','hidden'); //按下送出後都先將所有訊息隱藏
		
		if(USERNAME.length < 1){
			$('#con_error_username').css('visibility','visible');
			document.getElementById("con_error_username").innerHTML = "暱稱不能為空喔！";
			check = false;
		}
		else{
			if(!USERNAME.match(username_format)){
				$('#con_error_username').css('visibility','visible');
				document.getElementById("con_error_username").innerHTML = "不能使用注音　符號只能使用-或_";
				check = false;
			}
		}
		
		if(EMAIL.length < 1){
			$('#con_error_email').css('visibility','visible');
			document.getElementById("con_error_email").innerHTML = "信箱不能為空！";
			check = false;
		}
		else{
			if(!EMAIL.match(mail_format)){
				$('#con_error_email').css('visibility','visible');
				document.getElementById("con_error_email").innerHTML = "信箱格式不正確";
				check = false;
			}
		}
		
		if(CATEGORY=="請選擇類別..."){
			$('#con_error_category').css('visibility','visible');
			document.getElementById("con_error_category").innerHTML = "請選擇一項類別";
			check = false;
		}
		
		if(SUBJECT.length < 1){
			$('#con_error_subject').css('visibility','visible');
			document.getElementById("con_error_subject").innerHTML ="還沒有填標題喔！";
			check = false;
		}
		else{
			 if(!SUBJECT.match(sub_con_format)){
				 $('#con_error_subject').css('visibility','visible');
				 document.getElementById("con_error_subject").innerHTML ="還沒有填標題喔！";
				 check = false;
			}
		...
		if(CONTENT.length < 1){
			$('#con_error_content').css('visibility','visible');
			document.getElementById("con_error_content").innerHTML ="還沒有填內容喔！";
			check = false;
		}
		else{
			 if(!CONTENT.match(sub_con_format)){
				 $('#con_error_content').css('visibility','visible');
				 document.getElementById("con_error_content").innerHTML ="還沒有填內容喔！";
				 check = false;
			}
		...
		if(check===true){
			//alert("WRRRYYYY!!!!!!");
			$(".contact-card *").prop('disabled',true); //按下送出後把contact-card所有元素的功能都停止
			$('.contact-submit').text('傳送中..');
			
			$.ajax({
				type:'POST',
				url:'/fortestbeta/contact-us.php',
				data:{  username:USERNAME , email:EMAIL , category:CATEGORY ,  subject:SUBJECT  , content:CONTENT  },
				//dataType: "json",
				success:function(response){
					...
					if(response==="success"){
						...
						$('.contact_finished').css('display','block');
						$(".contact-card *").prop('disabled',false); //開啟所有功能
						$('.contact-submit').text('送出');
						$(".contact-card input").val('');//清除全部輸入框
						$(".contact_content").val('');
						$('.contact_category').val('choose');//設定為預設值(請選擇類別)
					}
					else{
						$('#con_error_submit').css('visibility','visible');
						document.getElementById("con_error_submit").innerHTML ="送出失敗，請再試一次";
						$(".contact-card *").prop('disabled',false); //開啟所有功能
					}
				}/*,
				error: function(data){
	...
	function new_post(img , img_type ,upload_type){
		...
		if(upload_type === 'file'){
			...
			thread_imgurl = img;
			//image.crossOrigin = 'Anonymous'; //重要
			//alert(img);
			image.src = '/'+img;
			//image.src = '/temp-pic/test.jpg';
			//alert(image.crossOrigin+"~!");
			if(img_type==='image/gif'){
		...
		image.onerror = function() { //GIF有可能因為副檔名是webp所以onerror
			//console.log("error");
			if(img_type==='image/gif'){
				top_fontsize = 36;
				bottom_fontsize = 36;
				...
				$('.uploadimg_url-error').text('出現錯誤，請再試一次');
			}
		}
	}
	...
	$(document).on('paste keyup', '.thread_uploadimg_url', function() {
		...
		globalTimeout_thread = setTimeout(function() {
			...
			//if(img_url.length>6){
				...
				$.ajax({
					type:'POST',
					url:'/fortestbeta/uploadimg-url-check.php',
					...
					success:function(response){
						...
						//alert(response);
						var response = response.trim();
						if(response==="image/gif"){
							var img_type = response;
							$('.uploadimg_url-error').css('visibility','hidden');
							$('.uploadimg_url-error').text('URL');
							new_post(img_url , img_type , 'url');
						}
						else if(response==="image/png" || response==="image/jpeg" || response==="image/jpg" ){
							var img_type = response;
							urlimg_savetotemp(img_url , img_type , 'url'); //因為canvas如果用URL會有CORS問題。所以先存到temp才到發文介面
						}
	...
	function urlimg_savetotemp(img_url , img_type ,upload_type){
		$.ajax({
			type:'POST',
			url:'/fortestbeta/imgurl-temp.php',
			data:{image_url : img_url},
			success:function(response){
				//var response = response.trim();
				//alert(response);
				if(response===false){
					...
					var img_url_temp = response.trim();
					//alert(img_url_temp+"~"+img_type+"~~"+upload_type);
					new_post(img_url_temp , img_type , upload_type);
					thread_imgurl_temp = img_url_temp;
					$('.uploadimg_url-error').css('visibility','hidden');
					$('.uploadimg_url-error').text('URL');
				}
			}/*,
	...
	function urlimg_savetotemp_delete(img_url){
		$.ajax({
			type:'POST',
			url:'/fortestbeta/imgurl-temp-delete.php',
			data:{thread_imgurl_temp : img_url},
			success:function(response){
				//alert(response);
			}/*,
			error: function(data){
	...
	$(document).on('click', '.newthread_post-close', function() {
		if(thread_imgtype==="image/png" || thread_imgtype==="image/jpeg" || thread_imgtype==="image/jpg" ){
			urlimg_savetotemp_delete(thread_imgurl_temp);
		}
		$('body').css('overflow','auto');
		...
		if(SUBJECT.length < 1){
		 ...
		 if(format_correct==true){
			...
			if(topline_text.length>0 || bottomline_text.length>0){
				...
				formdata.append('thread_imgurl_temp', thread_imgurl_temp);
			}
			else{
			...
			$.ajax({
				type:'POST',
				url:'/fortestbeta/newthread-db.php',
				data:formdata,
				...
				success:function(response){
					...
					if(response.match(threadid_format)){
						window.location = "/fortestbeta/thread-show.php?thread_id="+response;
					}
					else{
						alert("發文失敗喔！ 請再嘗試一次");
	...
	var get_url_hash = window.location.hash.substr(1).split('&');
	var get_accessToken = get_url_hash[0].split('='); 
	//geturl_hash[0] = (accessToken=得到的token)
	//get_accessToken[0] = accessToken
	//get_accessToken[1] = 得到的token數據
	if(get_accessToken[0]==='access_token'){
		var FB_ACCESS_TOKEN = get_accessToken[1];
		//alert(FB_ACCESS_TOKEN);
		$.ajax({
			type:'POST',
			url:'/fortestbeta/register-fb-manually.php',
			data:{fb_access_token : FB_ACCESS_TOKEN},
			success:function(response){
				//alert(response);
				var response = response.trim();
				if(response==='success'){
					//window.location = "/fortestbeta/profile.php";
					facebook_reg_manually(); /*FACEBOOK註冊-手動*/
				}
				else{
					alert('facebook登入失敗，請再試一次');
				}
			}/*,
			error: function(data){
			//if(data.success == true){alert("失敗");}
	...
	/*FACEBOOK註冊-手動*/
	function facebook_reg_manually(){
		FB.getLoginStatus(function(response) {
		  if (response.status === 'connected') {
			...
			FB.api('/me', {fields: 'name'}, function(response) {
				...
				$.ajax({
					type:'POST',
					url:'/fortestbeta/register-fb-to-db.php',
					...
					success:function(response){
						...
						if(response==='success_reg'){
							...
							window.location = "/fortestbeta/index.php";
						}
			  ...
			  alert('您尚未點選確認新增我們的應用程式');
			// the user is logged in to Facebook, 
			// but has not authenticated your app
		  } else {
			   alert('您還沒有登入acebook，請再試一次');
	...
	$(document).on('click','.facebook_reg',function(){
		//alert('~~');
		//window.location = 'https://www.facebook.com/v2.10/dialog/oauth?client_id=1895947000621168&redirect_uri=http://llamafun.com&response_type=token';
		//alert('123');
	...
	function messageCount(){
		$.ajax({
			type:'GET',
			url:'/fortestbeta/message-count.php',
...
//	$(".testdiv").nanoScroller();
	...
	$(document).on('click','#notifylink',function() // onclick function for notification
	{
		...
		if(newmsg==true){
			...
			$.ajax({
				type:'POST',
				url:'/fortestbeta/message-show.php',
	...
	$(document).on('click','.side-nft-msg',function(){
		$.ajax({
			url:'/fortestbeta/message-side-nft.php',
			success:function(response){
				//alert(response);
			}/*,
			error: function(data){
	...
	$(document).on('click','.nft-readall',function(){
			$.ajax({
				url:'/fortestbeta/message-read-all.php',
				success:function(response){
	...
	$(document).on('click','.nft-list',function(){
		...
		*/
		...
		if(READ_NFT=='0'){
			...
			$.ajax({
				type:'POST',
				url:'/fortestbeta/message-read.php',
				data:{nft_id : NFT_ID},
				success:function(response){
					//alert(response);
					//$('#notificationsBody').prepend(response);
	...
	$(document).on('click','.comment-nft-onoff',function(){
		...
		$.ajax({
			type:'POST',
			url:'/fortestbeta/comment-nft-onoff.php',
...
$("body").click(function(){
				...
				if( $('.notifications_page_body .nft_loding').length ){
					...
					*/
					...
					if(NFTLOAD_FINISH===true){
						...
						$.ajax({
							type:'POST',
							url:'/fortestbeta/message-show-more.php',
							data:{nft_date:NFT_DATE},
							success:function(html){
	...
	$("#notificationFooter").click(function(){
		window.location = "/fortestbeta/member_notifications.php";
	});
		...
		if(USERNAME.length < 1){
			...
			if(USERNAME.length > 12){
				...
				if(!USERNAME.match(username_format)){
					...
					$.ajax({
						type:'POST',
						url:'/fortestbeta/register-check-username.php',
						data:{username:USERNAME},
						success:function(response){
							if(response==true){
	...
	$(document).on('keyup change', '#register_email', function() {
		...
		if(EMAIL.length < 1){
			...
			if(!EMAIL.match(mail_format)){
				...
				$.ajax({
					type:'POST',
					url:'/fortestbeta/register-check-email.php',
					data:{email:EMAIL},
					success:function(response){
						if(response==true){
	...
	$(document).on('click','.popup_registerbtn',function(){
		...
		if(reg_username_correct == true && reg_email_correct == true && reg_pw_correct == true && reg_pw_recheck_correct == true){
			if(reCAPTCHA_REG_STATUS===false){
				$('#register_message').css('visibility','visible');
				document.getElementById("register_message").innerHTML = "請點選驗證";
			}
			else if(reCAPTCHA_REG_STATUS===true){
				var USERNAME = document.getElementById("register_username").value;
				var EMAIL = document.getElementById("register_email").value;
				var PASSWORD = document.getElementById("register_password").value;
				$.ajax({
					type:'POST',
					url:'/fortestbeta/register-to-db.php', 
					data:{  username : USERNAME , email : EMAIL , password : PASSWORD ,  recaptcha_reg_callback : reCAPTCHA_REG_CALLBACK },
					//dataType: "json",
					success:function(response){
						if(response==true){
							reCAPTCHA_REG_STATUS=false;
							reCAPTCHA_REG_CALLBACK = null;
							...
							reCAPTCHA_REG_STATUS=false;
							reCAPTCHA_REG_CALLBACK = null;
							grecaptcha.reset(reCAPTCHA_REG);//重置機器人驗證
							$('#register_message').css('visibility','visible');
	...
	$(document).on('click','.popup_loginbtn',function(){
		...
		if(all_format_correct == true){
			if(reCAPTCHA_LOGIN_STATUS===false){
				$('#login_message').css('visibility','visible');
				document.getElementById("login_message").innerHTML = "請點選驗證";
			}
			else if(reCAPTCHA_LOGIN_STATUS===true){
				$.ajax({
				type:'POST',
				url:'/fortestbeta/login-db.php',  
				data:{  email : EMAIL , password : PASSWORD , recaptcha_login_callback :reCAPTCHA_LOGIN_CALLBACK },
				//dataType: "json",
				success:function(response){
					if(response==true){
						reCAPTCHA_LOGIN_STATUS=false;
						reCAPTCHA_LOGIN_CALLBACK = null;
						$( "#login_form" ).submit();
						//location.reload();
						...
						reCAPTCHA_LOGIN_STATUS=false;
						reCAPTCHA_LOGIN_CALLBACK = null;
						grecaptcha.reset(reCAPTCHA_LOGIN);//重置機器人驗證
						$('#login_message').css('visibility','visible');
	...
	$(document).on('click','.forgotpw',function(){
		$('.login_popup').css('display','none');
		$('#forgotpw_message').css('visibility','hidden');
		document.getElementById("forgotpw_message").innerHTML = "MSG";
		$('.forgotpw-finished').css('display','none');//每次開啟先關閉forgotpw-finished
		$('.forgotpw-card').css('display','block');
		$('.forgotpw_popup').css('display','block');
		
	});
	//關閉忘記密碼
	$(document).on('click','.forgotpw-card-close',function(){
		$('body').css('overflow','auto');
		$('.forgotpw_popup').css('display','none');
	});
	//忘記密碼送出
	$(document).on('click','.forgotpw-submit',function(){
		var mail_format = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
		var EMAIL = document.getElementById("forgotpw_email").value;
		var all_format_correct = true;

		if(EMAIL.length < 1){
			$('#forgotpw_message').css('visibility','visible');
			document.getElementById("forgotpw_message").innerHTML = "請輸入信箱";
			all_format_correct = false;
		}
		else{
			if(!EMAIL.match(mail_format)){
				$('#forgotpw_message').css('visibility','visible');
				document.getElementById("forgotpw_message").innerHTML = "信箱格式不正確";
				all_format_correct = false;
			}
		}
		...
		if(all_format_correct == true){
				$('#forgotpw_message').css('visibility','visible');
				document.getElementById("forgotpw_message").innerHTML = "處理中...";
				$(".forgotpw-card *").prop('disabled',true); //關閉全部功能
				
				//$(".forgotpw_popup").fadeOut(4000);
				//$('body').css('overflow','auto');auto
				$.ajax({
				type:'POST',
				url:'/fortestbeta/forgotpw.php', 
				data:{  email : EMAIL },
				//dataType: "json",
				success:function(response){
					var response = response.trim();
					//alert(response);
					if(response==="success"){
						$('.forgotpw-card').css('display','none');
						$('#forgotpw_email').val('');
						$('.forgotpw-finished').css('display','block');
						$(".forgotpw-card *").prop('disabled',false); //開啟全部功能
					}
					else if(response==="notfound"){
						document.getElementById("forgotpw_message").innerHTML = "此用戶不存在";
						$(".forgotpw-card *").prop('disabled',false); //開啟全部功能
					}
					else if(response==="facebook"){
						document.getElementById("forgotpw_message").innerHTML = "因使用FB註冊，並無登入密碼";
						$(".forgotpw-card *").prop('disabled',false); //開啟全部功能
					}
					else{
						document.getElementById("forgotpw_message").innerHTML = "送出失敗、請再試一次";
						$(".forgotpw-card *").prop('disabled',false); //開啟全部功能
					}
				}/*,
				error: function(data){
  ...
  .on('drop', function(e) {
	...
	$(document).on('click','.likebtn',function(){
		...
		if(LIKE_UNLILE === "likebtn"){
			if(LOGGEDIN_CHECK!=true){
				...
				$.ajax({
					type:'POST',
					url:'/fortestbeta/thread-like.php',
			...
			$.ajax({
				type:'POST',
				url:'/fortestbeta/thread-unlike.php',
				data:{  thread_id : THREAD_ID , member_id : MEMBER_ID },
				success:function(html){
					$('#likecount_'+THREAD_ID).replaceWith(html);
				}
	...
	$(document).on('click', '.thread-com-report', function() {
		...
		*/
		...
		if(LOGGEDIN_CHECK!=true){	//若沒登入跳出登入視窗
			...
			$('.report_finished').hide(); //因為送出後會出現這個report_finished，所以每次按下都先隱藏
	...
	$(document).on('click', '.report_popup-close', function() {
		$('body').css('overflow','auto');
		$('.report_popup').hide();
		$('.popup_report-next-unchecked').removeClass( "popup_report-next report_submitting" );
		$(".popup_report-next-unchecked").text('送出');
		$('.report-radio-other').val('');
		$(".report-radio").find("input:radio").prop("checked", false).end().buttonset("refresh");
	});
	...
	$(document).on('click', '.popup_report-next', function() {
		...
		$(".report-card *").prop('disabled',true); //按下送出後把report-card所有元素的功能都停止
		$(".popup_report-next-unchecked").addClass("report_submitting");
		$(".popup_report-next-unchecked").text('傳送中..');
		
		$.ajax({
			type:'POST',
			url:'/fortestbeta/send-report.php',
			data:formdata,
			processData: false,  // tell jQuery not to process the data
			contentType: false,
			success:function(response){
				...
				if(response==true){
					...
					$(".report-card *").prop('disabled',false); //開啟全部功能
					$('.popup_report-next-unchecked').removeClass( "popup_report-next report_submitting" );
					$(".popup_report-next-unchecked").text('送出');
					$('.report-radio-other').val('');
					$(".report-radio").find("input:radio").prop("checked", false).end().buttonset("refresh");
					...
					alert('送出失敗，請再試一次');
					$(".report-card *").prop('disabled',false); //開啟全部功能
					$('.popup_report-next-unchecked').removeClass( "report_submitting" );
					$(".popup_report-next-unchecked").text('送出');
				}
			}/*,
			error: function(data){
	...
	$(document).on('keyup paste', '.comment-image-url', function() {
		comment_image_check = false;//一開始都先設定為false、若格式接正確才會是true
		...
		*/
		...
		if(LOGGEDIN_CHECK != true){
			...
			if(format_correct === true){
				...
				$.ajax({
					type:'POST',
					url:'/fortestbeta/send-comment.php',
					data:formdata,
					processData: false,  // tell jQuery not to process the data
					contentType: false,
					success:function(response){
						...
						if(response!=false){
							...
							$('#div-comments_'+THREAD_ID).show(); //因為RWD隱藏留言所以要設定show
...
/*	$('.comment-gif-video').parent().click(function () {
	...
	$(document).on('click','.show_more',function(){
		...
		*/
		...
		$.ajax({
			type:'POST',
			url:'/fortestbeta/show-more-comments.php',
	...
	$(document).on('click','.reply_showmore',function(){
		...
		*/
		...
		$.ajax({
			type:'POST',
			url:'/fortestbeta/show-more-reply.php',
	...
	$(document).on('keyup paste', '.reply-image-url', function() {
		reply_image_check = false;//一開始都先設定為false、若格式接正確才會是true
		...
		if(LOGGEDIN_CHECK != true){
			...
			if(reply_format_correct===true){
				...
				*/
				...
				$.ajax({
					type:'POST',
					url:'/fortestbeta/comment-reply.php',
					data:formdata,
					processData: false,  // tell jQuery not to process the data
					contentType: false,
					success:function(response){		
		...
		*/
		...
		if(COMLIKE_UNLILE==="comments-likebtn"){
			if(LOGGEDIN_CHECK != true){
				...
				$.ajax({
					type:'POST',
					url:'/fortestbeta/comments-like.php',
			...
			$.ajax({
				type:'POST',
				url:'/fortestbeta/comments-unlike.php',
				data:{  reply_com_id : REPLY_COM_ID  , comments_member_id:COM_MEMBER_ID},
				success:function(html){
					$('#commentslikecount_'+REPLY_COM_ID).replaceWith(html);
				}
	...
	$(document).on('click','.thread-nft-onoff',function(){
		...
		$.ajax({
			type:'POST',
			url:'/fortestbeta/thread-nft-onoff.php',
			data:{  thread_id : THREAD_ID , nft_onoff : NFT_ONOFF},
			success:function(response){
				//alert(response);
				if(response==true){
	...
	$(document).on('click','.thread-edit',function(){
		...
		else if($( ".thread_show" ).length ){
			$('.thread-edit-submit').remove();
			$('.textarea_subject_edit').replaceWith("<div id='thread-title_"+EDIT_THREAD_ID+"' class='div-row thread_title'>"+EDIT_SUBJECT+"</div>");
		}
	...
	$(document).on('click','.thread-edit-submit',function(){
		...
		if(format_correct == true){
			...
			$.ajax({
				type:'POST',
				url:'/fortestbeta/thread-edit.php',
	...
	$(document).on('click','.thread-delete',function(){
		...
		if (confirm("確定要刪除文章嗎？")) {
			...
			*/
			...
			$.ajax({
				type:'POST',
				url:'/fortestbeta/thread-delete.php',
	...
	$(document).on('click','.comments-delete',function(){
		...
		if (confirm("確定要刪除留言嗎？")) {
			...
			*/
			...
			$.ajax({
				type:'POST',
				url:'/fortestbeta/comments-delete.php',
		...
		if(EDIT_USERNAME.length < 1){
			...
			if(EDIT_USERNAME.length > 12){
				...
				if(!EDIT_USERNAME.match(username_format)){
					...
					$.ajax({
						type:'POST',
						url:'/fortestbeta/profile-db.php',
